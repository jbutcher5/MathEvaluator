...
bool test(List<std::string>* input, List<std::string>* RPN, List<double>* result){
  if (input->size() != RPN->size() || input->size() != result->size() || RPN->size() != result->size()){
    ...
    for (int x = 0; x < (int)input->size(); x++){
      ...
      // Accept floating point error numbers.
      if (evaluation > result->getData(x)-.01 || evaluation < result->getData(x)+.01){
...
int main(){
  ...
  double result_arr[] = {2, 1, 28, 595, 36, 39, 32, 6.28, 92, 1, 2, 10, 13, 3628800, 3.14159};
